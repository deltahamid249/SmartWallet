...
$csrfToken = csrfToken();
$actionToken = actionToken('electricity_request');

$successMessage = '';
$errorMessage = '';

$providers = [
    'sudani_electricity' => 'شركة الكهرباء',
    'khartoum_electricity' => 'كهرباء الخرطوم',
    'other' => 'مزود آخر',
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!consumeActionToken('electricity_request', $_POST['action_token'] ?? '')) {
        ...
        if (!array_key_exists($provider, $providers)) {
            throw new RuntimeException('يرجى اختيار شركة الكهرباء.');
        }

        if ($accountNumber === '') {
            throw new RuntimeException('يرجى إدخال رقم الحساب أو العداد.');
        }
        ...
        $reference = 'ELC-' . strtoupper(bin2hex(random_bytes(6)));
        ...
        createAdminNotification(
            'طلب دفع كهرباء جديد',
            'تم إنشاء طلب دفع كهرباء جديد بالمرجع ' . $reference . '.',
            'service_request',
            'service_request',
            $requestId
        );
        ...
        flash(
            'success',
            'تم إنشاء طلب الكهرباء بنجاح. رقم المرجع: ' . $reference
        );
        ...
        error_log('Electricity request error: ' . $e->getMessage());

        $errorMessage = $e instanceof RuntimeException
            ? $e->getMessage()
            : 'حدث خطأ أثناء إنشاء طلب الكهرباء. حاول مرة أخرى.';
    }
}
...
<div class="container">
    ...
    <div class="notice">
        ⚠️ يتم حاليًا تسجيل طلب دفع الكهرباء ومتابعته من لوحة الإدارة.
        التنفيذ الآلي الفعلي يحتاج إلى تكامل رسمي مع شركة الكهرباء أو
        مزود دفع معتمد.
    </div>

    <div class="card">

        <h2>إنشاء طلب دفع كهرباء</h2>
            ...
            <div class="grid">
                ...
                <div>
                    ...
                    <input
                        ...
                        name="account_number"
                        maxlength="100"
                        placeholder="أدخل رقم الحساب أو العداد"
                        required
                    >
                </div>
    ...
    <div class="card">
        ...
        <h2>📋 طلبات الكهرباء السابقة</h2>

        <?php if (!$requests): ?>

            <div class="empty">
                لا توجد طلبات كهرباء حتى الآن.
            </div>
